...
void ReceiveFrameStateMachine(void)//  ***206 Begin
{
	int aChar;
	int rrv=0xff;
...
ResetToIDLE:
		...
		switch (mstpvars.lrstate)       //handle it based on present state
		{
			...
			case lrsmHeader: // 2
				...
				//mstpvars.hcrc = CalcHeaderCRC(mstpvars.hb[mstpvars.hindex++],mstpvars.hcrc);
            ...
            //mstpvars.dcrc = CalcDataCRC(mstpvars.rbuf[mstpvars.hindex++],mstpvars[port].dcrc);
            ...
            {
                //[DATA_CRC]-----------------------------------------------------------------------
                if (mstpvars.dcrc == 0xF0B8) {

                    mstpvars.mflag&=~receivedinvalidframe; // ***513    ***701
                    mstpvars.mflag|=receivedvalidframe;
