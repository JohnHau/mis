...
uint32_t parse_lpspi_cfg(cJSON* json,rt_lpspi_master_cfg_t* lpspi_cfg)
{
	...
	else if (CMP_STRING(jsonV->valuestring,"kLPSPI_ClockPhaseSecondEdge")) 
	{

		lpspi_cfg->cpha = kLPSPI_ClockPolarityActiveHigh;
		printf("cpha value is  %d\n",lpspi_cfg->cpha);
		printf("cpha is  %s\n",jsonV->valuestring);
	}
	else if (CMP_STRING(jsonV->valuestring,"kLPSPI_MsbFirst")) 
	{
		lpspi_cfg->direction = kLPSPI_ClockPolarityActiveHigh;
		printf("direction value is  %d\n",lpspi_cfg->cpol);
		printf("direction is  %s\n",jsonV->valuestring);
	}
	else if (CMP_STRING(jsonV->valuestring,"kLPSPI_Pcs0")) 
	{

		lpspi_cfg->whichPcs= kLPSPI_ClockPolarityActiveHigh;
		printf("whichPcs value is  %d\n",lpspi_cfg->whichPcs);
		printf("whichPcs is  %s\n",jsonV->valuestring);
	}
	else if (CMP_STRING(jsonV->valuestring,"kLPSPI_PcsActiveLow")) 
	{

		lpspi_cfg->pcsActiveHighOrLow = kLPSPI_PcsActiveLow;
		printf("pcsActiveHighOrLow value is  %d\n",lpspi_cfg->cpol);
		printf("pcsActiveHighOrLow is  %s\n",jsonV->valuestring);
	}
	else if (CMP_STRING(jsonV->valuestring,"kLPSPI_SdiInSdoOut")) 
	{
		lpspi_cfg->pinCfg= kLPSPI_ClockPolarityActiveHigh;
		printf("pinCfg value is  %d\n",lpspi_cfg->cpol);
		printf("pinCfg is  %s\n",jsonV->valuestring);
	}
	else if (CMP_STRING(jsonV->valuestring,"kLpspiDataOutRetained")) 
	{

		lpspi_cfg->dataOutConfig = kLPSPI_ClockPolarityActiveHigh;
		printf("dataOutConfig value is  %d\n",lpspi_cfg->cpol);
		printf("dataOutConfig is  %s\n",jsonV->valuestring);
	}
	else
	{
		return -1;
